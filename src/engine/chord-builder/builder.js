...
function getNote(model, distance, role) {
  //console.log(model, distance, role)
...
class ChordBuilder {
  ...
  buildChord(model) {
    ...
    if (this.model.root) {
      ...
      if (fifth) {
        const isFifth = item =>
          item.interval === 5 ||
          (item.interval === 6 && item.accidental === 'b')
        const fifthModified = find(isFifth)(this.model.extra)
        if (fifthModified) {
          fifth = adjustWithAccidental(
            fifth,
            fifthModified.interval === 6 ? '#' : fifthModified.accidental
          )
        }
      ...
      each(item => {
        ...
        const isB6 = item.interval === 6 && item.accidental === 'b'
        if (isAdd && item.interval !== 5 && !isB6) {
  ...
  setMidiNotes() {
    decorateNotes(this.model.notes)
    let octave
    if (this.model.bass) {
      ...
      this.model.bassNote = {
        ...
        vexNote: formatNote(
          this.model.bass,
          this.model.bassModifier ? this.model.bassModifier : '',
          octave
        )
      }
  ...
  buildJazzChord() {
    ...
    if (
      ...
      this.model.extra &&
      this.model.extra.filter(x => x.interval === 5 && x.accidental === 'b')
        .length > 0
    ) {
      // half dim
      voicing.push(getNote(this.model, 0, 'root'))
      voicing.push(getNote(this.model, 5, 'third'))
      voicing.push(getNote(this.model, 6, 'fifth'))
      voicing.push(getNote(this.model, 10, 'seventh'))
    } else if (
      ['m', 'maj'].includes(this.model.quality) &&
      [7, 9].includes(this.model.interval)
    ) {
      // 9 voicing
      voicing.push(
        getNote(this.model, this.model.quality === 'maj' ? 11 : 10, 'seventh')
      )
      voicing.push(getNote(this.model, 14, 'ninth'))
      voicing.push(
        getNote(this.model, this.model.quality === 'm' ? 3 : 4, 'third')
      )
      voicing.push(getNote(this.model, 7, 'fifth'))
    } else if (
      this.model.quality === 'alt' ||
      (!this.model.quality &&
        this.model.interval === 7 &&
        (this.model.extra || []).filter(
          x =>
            (x.interval === 9 && x.accidental == '#') ||
            (x.interval === 13 && x.accidental == 'b')
        ).length > 0)
    ) {
      // alt
      voicing.push(getNote(this.model, 4, 'third'))
      voicing.push(getNote(this.model, 8, 'thirteenth'))
      voicing.push(getNote(this.model, 10, 'seventh'))
      voicing.push(getNote(this.model, 15, 'ninth'))
    } else if (
      !this.model.quality &&
      [7, 9, 13].includes(this.model.interval) &&
      !this.model.extra
    ) {
      // 13 voicing
      voicing.push(getNote(this.model, 4, 'third'))
      voicing.push(getNote(this.model, 9, 'sixth'))
      voicing.push(getNote(this.model, 10, 'seventh'))
      voicing.push(getNote(this.model, 14, 'ninth'))
    }
    decorateNotes(voicing)
    this.model.voicing = voicing.length ? voicing : null
  }
  buildGuitarChord() {
    ...
    const assignString = (j, chord, onlyMainNotes) => {
      ...
      const smallestDistance = flow(
        map(note => {
          const distance =
            getDistance(freeNote, note) +
            (isSuitableForGuitarString(note, j) ? 0 : 36) +
            (onlyMainNotes && note.role !== 'root' && note.role !== 'fifth'
              ? 36
              : 0)
    ...
    function clearNonRoots() {
      ...
      flow(
        filter(
          item =>
            notMute(item) && item.index === lowest && item.note.role !== 'root'
        ),
        map('index'),
        each(index => {
          tab[index] = emptyTab()
